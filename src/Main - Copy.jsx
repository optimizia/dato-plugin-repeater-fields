...
export default class Main extends Component {
  ...
  componentDidMount() {
    ...
    const client = new SiteClient('5427be16326d15058076a6e4d6481a');
    if (this.props.fieldValue !== null) {
      this.setState({ formattedContent: JSON.parse(this.props.fieldValue), links: [] }, () => {
        ...
        let i = 0;
        this.state.formattedContent.map((content, mainPosition) => {
      ...
      const newFieldArray = fieldArray.map((field, key) => {
        // console.log(plugin)
        if (field.type === 'field') {
          // console.log(mainPosition)
          const data = client.fields.find(field.id)
            .then((itemType) => {
              // console.log(itemType)
              let tempArray = []
              const field_type = itemType.fieldType
              const label = itemType.label
              const api_key = itemType.apiKey
              const value = itemType.fieldType === 'links' ? [] : (itemType.defaultValue ? itemType.defaultValue : '')
              const links = itemType.fieldType === 'links' && itemType.validators.itemItemType.itemTypes
              const validators = itemType.field_type === 'string' && itemType.validators && itemType.validators.enum && itemType.validators.enum.values && itemType.validators.enum.values

              compiledBlock.push({
                field_type : itemType.fieldType,
                label : itemType.label,
                api_key : itemType.apiKey,
                value : itemType.fieldType === 'links' ? [] : (itemType.defaultValue ? itemType.defaultValue : ''),
                links : itemType.fieldType === 'links' && itemType.validators.itemItemType.itemTypes,
                validators : itemType.field_type === 'string' && itemType.validators && itemType.validators.enum && itemType.validators.enum.values && itemType.validators.enum.values,
                order: key,
              })
              ...
              if(key == fieldArray.length-1 )
              {
                // console.log(fieldArray.length-1)
                i = i + 1;
                const collection = compiledBlock
                // collection.sort(function (a, b) {
                //   return a.order - b.order;
                // });
                // console.log(collection)
                // collection.push({
                //   field_type, label, api_key, value, links, validators
                // });

                const newContent = this.state.allContent;
                newContent.push({
                  // model_id: modelID, id, name: contentName, fields: collection, api_key: contentApiKey,
                  model_id: content.model_id, id: content.id, name: contentName, fields: collection, api_key: contentApiKey, order:mainPosition
                });
                // console.log(collection)
  ...
  render() {
    ...
    const createFields = (id, fieldContent) => {
      // console.log(plugin.field.attributes.appearance.parameters)
    ...
    const getField = (id, callback) => {
      ...
      .then((itemType) => {
        // this.setState({allFields:itemType}
      callback(itemType);
      })
    }
    const handleOptionSelected = (modelID) => {
      ...
      // console.log(plugin.fields[fieldArray.length-1])
      // console.log(fieldArray)
      // const newFieldArray1 = fieldArray.map((field) => {
      //   if (field.type === 'field') {
      ...
      //     // console.log(plugin.fields)
      //     // if(plugin.fields[field.id])
      //     // {
      //     //   const fieldContent = plugin.fields[field.id].attributes;
      //     //   // console.log(fieldContent)

      //     //   const field_type = fieldContent.field_type
      //     //   const label = fieldContent.label
      //     //   const api_key = fieldContent.api_key
      //     //   const value = fieldContent.field_type === 'links' ? [] : (fieldContent.default_value ? fieldContent.default_value : '')
      //     //   const links = fieldContent.field_type === 'links' && fieldContent.validators.items_item_type.item_types
      //     //   const validators = fieldContent.field_type === 'string' && fieldContent.validators && fieldContent.validators.enum && fieldContent.validators.enum.values && fieldContent.validators.enum.values
      ...
      //     // else
      //     // {
      //     //   // console.log(getField(field.id))
      //     //   // getField(field.id, function(returnValue) {
      //     //   //   const itemType = returnValue
      //     //   //   this.setState({allFields: itemType}, () => {
